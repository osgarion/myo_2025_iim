# ************************************************************
# FI2 vs GGIR explanatory value for SF-36 PCS
...
prepare_fi2_ggir_eff_data_vis07 <- function(ggir_data) {
  ...
  fi2_col <- resolve_first_existing_vis06(ggir_data, c("fi2", "fi_2"), "fi2")
...
build_models_for_parameter_vis07 <- function(data, ggir_parameter, log_outcome = FALSE) {
  dat <- data |>
    select(immet_id, exam, age_m0, sex, sf36_pcs, sf36_mcs, fi2, all_of(ggir_parameter)) |>
    na.omit()
  ...
  formulas <- list(
    fi2_only = stats::as.formula(
      paste0(
        outcome_expr,
        " ~ fi2 + exam + age_m0 + sex + sf36_mcs + (1 | immet_id)"
      )
    ),
    ...
    fi2_ggir = stats::as.formula(
      paste0(
        outcome_expr,
        " ~ fi2 + ", ggir_parameter, " + exam + age_m0 + sex + sf36_mcs + (1 | immet_id)"
      )
    )
  )
...
build_compare_plot_vis07 <- function(models_named, ggir_parameter, log_outcome = TRUE) {
  valid_models <- models_named[!purrr::map_lgl(models_named, is.null)]
  ...
  if (inherits(p_obj, "ggplot")) {
    ...
    p_obj <- p_obj +
      ggplot2::labs(
          ...
          subtitle = paste0(
          outcome_label, " ~ fi2 + ", ggir_parameter,
...
build_actual_vs_pred_plot_vis06 <- function(pred_df, ggir_parameter, log_outcome = TRUE) {
  ...
  ggplot(pred_df, aes(x = observed, y = predicted, color = model)) +
    ...
    labs(
      ...
      subtitle = paste0("log(sf36_pcs) models with fi2 for ", ggir_parameter),
      x = outcome_label,
      y = pred_label,
      color = "Model"
    ) +
...
build_residual_plot_vis06 <- function(pred_df, ggir_parameter) {
  ggplot(pred_df, aes(x = model, y = residual, color = model)) +
    ...
    labs(
      ...
      subtitle = paste0("Residual beeswarm for ", ggir_parameter, " with fi2"),
      x = "Model",
      y = "Residuals",
      color = "Model"
    ) +
...
build_bootstrap_boxplot_vis06 <- function(bootstrap_tbl, ggir_parameter) {
  metric_long <- bootstrap_tbl |>
    ...
    mutate(
      ...
      model = factor(model, levels = c("fi2_only", "ggir_only", "fi2_ggir"))
    )
...
build_multi_panel_plot_vis07 <- function(pred_df, bootstrap_tbl, ggir_parameter, log_outcome = TRUE) {
...
export_parameter_results_vis06 <- function(
  ...
  dir.create(file.path("output", "fi2_ggir_eff", "tables"), recursive = TRUE, showWarnings = FALSE)
  dir.create(file.path("output", "fi2_ggir_eff", "figures"), recursive = TRUE, showWarnings = FALSE)

  prefix <- paste0(format(Sys.Date(), "%y%m%d"), "_", sanitize_name_vis06(ggir_parameter), "_fi2_ggir_eff")
  xlsx_file <- file.path("output", "fi2_ggir_eff", "tables", paste0(prefix, "_", suffix, ".xlsx"))
  plot_file <- file.path("output", "fi2_ggir_eff", "figures", paste0(prefix, "_", suffix, ".png"))
  diagnostic_file <- file.path("output", "fi2_ggir_eff", "figures", paste0(prefix, "_diagnostic_", suffix, ".png"))
...
run_fi2_ggir_eff_workflow_vis07 <- function(
  ...
  prepared_data <- prepare_fi2_ggir_eff_data_vis07(ggir_data)
  ...
  parameter_results <- purrr::map(ggir_available, function(ggir_parameter) {
    raw_bundle <- build_models_for_parameter_vis07(
      data = prepared_data,
      ggir_parameter = ggir_parameter,
      log_outcome = FALSE
    )

    log_bundle <- build_models_for_parameter_vis07(
      data = prepared_data,
      ggir_parameter = ggir_parameter,
      log_outcome = TRUE
    )
    ...
    plot_obj <- build_compare_plot_vis07(
    ...
    diagnostic_plot <- build_multi_panel_plot_vis07(
      pred_df = pred_df,
...
if (exists("d18_ggir_data")) {
  ggir_vis07_results <- run_fi2_ggir_eff_workflow_vis07(
    ggir_data = d18_ggir_data,
    export_results = isTRUE(getOption("myo.ggir.vis07.export", TRUE))
  )

  future::plan("sequential")

  ggir_vis07_complete_models <- ggir_vis07_results$complete_models

  message(
    "vis07 exported to: ",
    normalizePath(file.path("output", "fi2_ggir_eff"), winslash = "/", mustWork = FALSE)
  )
}
