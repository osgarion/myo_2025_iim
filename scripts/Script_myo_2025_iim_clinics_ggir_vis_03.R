...
# with each available sf36* outcome.
# ************************************************************
...
run_sf36_workflow_vis03 <- function(
    data = d18_ggir_data,
    sf36_outcomes = c(
      "sf36_pcs", "sf36_mcs", "sf36pf", "sf36rp", "sf36bp",
      "sf36gh", "sf36vt", "sf36sf", "sf36re", "sf36mf"
  ...
  dir.create(file.path("output", "sf36", "tables"), recursive = TRUE, showWarnings = FALSE)
  dir.create(file.path("output", "sf36", "figures"), recursive = TRUE, showWarnings = FALSE)
    ...
    if (isTRUE(export_results)) {
      ...
      export_paths <- list(
        fi2 = export_result_vis03(
          result = res_fi2,
          table_file = file.path("output", "sf36", "tables", paste0(prefix, "_fi2adj_", suffix, ".xlsx")),
          plot_file = file.path("output", "sf36", "figures", paste0(prefix, "_fi2adj_", suffix, ".png"))
        ),
        mmt8 = export_result_vis03(
          result = res_mmt8,
          table_file = file.path("output", "sf36", "tables", paste0(prefix, "_mmt8adj_", suffix, ".xlsx")),
          plot_file = file.path("output", "sf36", "figures", paste0(prefix, "_mmt8adj_", suffix, ".png"))
        ),
        single = export_result_vis03(
          result = res_single,
          table_file = file.path("output", "sf36", "tables", paste0(prefix, "_single_", suffix, ".xlsx")),
          plot_file = file.path("output", "sf36", "figures", paste0(prefix, "_single_", suffix, ".png"))
        )
      )
    }
...
if (exists("d18_ggir_data")) {
  ggir_vis03_results <- run_sf36_workflow_vis03(
    data = d18_ggir_data,
    export_results = isTRUE(getOption("myo.ggir.vis03.export", TRUE))
  )

  message("vis03 exported to: ", normalizePath(file.path("output", "sf36"), winslash = "/", mustWork = FALSE))
}
