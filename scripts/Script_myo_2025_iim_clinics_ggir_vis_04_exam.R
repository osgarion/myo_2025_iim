...
run_ggir_clinical_workflow_vis04_exam <- function(
    data = d18_ggir_data,
    ggir_outcomes = c(
      "ig_gradient_enmo",
      "ig_intercept_enmo",
  ...
  dir.create(file.path("output", "exam_incl", "ggir", "tables"), recursive = TRUE, showWarnings = FALSE)
  dir.create(file.path("output", "exam_incl", "ggir", "figures"), recursive = TRUE, showWarnings = FALSE)
  ...
  results <- map(ggir_available, function(outcome_name) {
    ...
    if (isTRUE(export_results)) {
      ...
      export_paths <- export_result_vis04(
        result = result,
        table_file = file.path("output", "exam_incl", "ggir", "tables", paste0(prefix, "_exam_", suffix, ".xlsx")),
        plot_file = file.path("output", "exam_incl", "ggir", "figures", paste0(prefix, "_exam_", suffix, ".png"))
      )
    }
...
if (exists("d18_ggir_data")) {
  ggir_vis04_exam_results <- run_ggir_clinical_workflow_vis04_exam(
    data = d18_ggir_data,
    export_results = isTRUE(getOption("myo.ggir.vis04.exam.export", TRUE))
  )

  message("vis04_exam exported to: ", normalizePath(file.path("output", "exam_incl", "ggir"), winslash = "/", mustWork = FALSE))
}
