...
# For each GGIR outcome fit models:
# ggir_parameter ~ tested clinical variable + age_m0 + sex + (1 | immet_id)
# ************************************************************
...
prepare_ggir_clinical_data_vis04 <- function(data) {
  ...
  fi2_col <- resolve_first_existing_vis04(data, c("fi2", "fi_2"), "fi2")
...
compare_clinical_predictors_for_ggir_vis04 <- function(data, response, clinical_parameters, ci = 0.95) {
  ...
  list(
    ...
    radar_plot = build_fmsb_radar_vis04(
      ...
      subtitle = paste0(response, " ~ tested variable + age_m0 + sex + (1 | immet_id)")
    )
  )
}
...
run_ggir_clinical_workflow_vis04 <- function(
    data = d18_ggir_data,
    ggir_outcomes = c(
      "ig_gradient_enmo",
      "ig_intercept_enmo",
  ...
  dir.create(file.path("output", "ggir", "tables"), recursive = TRUE, showWarnings = FALSE)
  dir.create(file.path("output", "ggir", "figures"), recursive = TRUE, showWarnings = FALSE)

  results <- map(ggir_available, function(outcome_name) {
    result <- compare_clinical_predictors_for_ggir_vis04(
      data = prepared_data,
    ...
    if (isTRUE(export_results)) {
      ...
      export_paths <- export_result_vis04(
        result = result,
        table_file = file.path("output", "ggir", "tables", paste0(prefix, "_", suffix, ".xlsx")),
        plot_file = file.path("output", "ggir", "figures", paste0(prefix, "_", suffix, ".png"))
      )
    }
...
if (exists("d18_ggir_data")) {
  ggir_vis04_results <- run_ggir_clinical_workflow_vis04(
    data = d18_ggir_data,
    export_results = isTRUE(getOption("myo.ggir.vis04.export", TRUE))
  )

  message("vis04 exported to: ", normalizePath(file.path("output", "ggir"), winslash = "/", mustWork = FALSE))
}
