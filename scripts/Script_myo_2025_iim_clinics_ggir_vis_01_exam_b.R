# ************************************************************
# PhVAS radar comparisons for GGIR / fi3 / mmt8 models
# Combines the original vis_01 and vis_02 logic, with exam included.
# ************************************************************
...
prepare_ggir_phvas_data_vis01_exam <- function(ggir_data) {
  immet_col <- resolve_first_existing_vis05(ggir_data, c("immet_id"), "immet_id")
  exam_col <- resolve_first_existing_vis05(ggir_data, c("exam_order", "exam"), "exam")
  sex_col <- resolve_first_existing_vis05(ggir_data, c("sex", "pohlavi", "gender"), "sex")
  age_col <- resolve_first_existing_vis05(ggir_data, c("age_m0", "age"), "age_m0")
  fi3_col <- resolve_first_existing_vis05(ggir_data, c("fi3", "fi_3"), "fi3")
  mmt8_col <- resolve_first_existing_vis05(ggir_data, c("mmt8", "mmt8_total"), "mmt8")
  phvas_col <- resolve_first_existing_vis05(ggir_data, c("ph_vas", "physician_vas", "PhVAS"), "ph_vas")
  ...
  ggir_cols <- intersect(ggir_candidates, names(ggir_data))
...
run_phvas_workflow_vis01_exam_b <- function(
  ...
  prepared_data <- prepare_ggir_phvas_data_vis01_exam(ggir_data)
  ...
  dir.create(file.path("output", "exam_incl", "fi3", "ph_vas", "tables"), recursive = TRUE, showWarnings = FALSE)
  dir.create(file.path("output", "exam_incl", "fi3", "ph_vas", "figures"), recursive = TRUE, showWarnings = FALSE)

  res_fi3 <- compare_adjusted_models_vis05(
    data = prepared_data,
    response = response,
    base_predictor = "fi3",
  ...
  if (isTRUE(export_results)) {
    prefix <- paste0(format(Sys.Date(), "%y%m%d"), "_", sanitize_name_vis05(response), "_phvas_fi3")

    export_paths <- list(
      fi3 = export_result_vis05(
        result = res_fi3,
        table_file = file.path("output", "exam_incl", "fi3", "ph_vas", "tables", paste0(prefix, "_fi3adj_exam_b_", suffix, ".xlsx")),
        plot_file = file.path("output", "exam_incl", "fi3", "ph_vas", "figures", paste0(prefix, "_fi3adj_exam_b_", suffix, ".png"))
      ),
      mmt8 = export_result_vis05(
        result = res_mmt8,
        table_file = file.path("output", "exam_incl", "fi3", "ph_vas", "tables", paste0(prefix, "_mmt8adj_exam_b_", suffix, ".xlsx")),
        plot_file = file.path("output", "exam_incl", "fi3", "ph_vas", "figures", paste0(prefix, "_mmt8adj_exam_b_", suffix, ".png"))
      ),
      single = export_result_vis05(
        result = res_single,
        table_file = file.path("output", "exam_incl", "fi3", "ph_vas", "tables", paste0(prefix, "_single_exam_b_", suffix, ".xlsx")),
        plot_file = file.path("output", "exam_incl", "fi3", "ph_vas", "figures", paste0(prefix, "_single_exam_b_", suffix, ".png"))
      )
    )
  }
...
if (exists("d18_ggir_data")) {
  ggir_vis01_exam_b_results <- run_phvas_workflow_vis01_exam_b(
    ggir_data = d18_ggir_data,
    export_results = isTRUE(getOption("myo.ggir.vis01.exam_b.export", TRUE))
  )

  res_phvas_exam_b_fi3 <- ggir_vis01_exam_b_results$fi3_adjusted
  res_phvas_exam_b_mmt8 <- ggir_vis01_exam_b_results$mmt8_adjusted
  res_phvas_exam_b_single <- ggir_vis01_exam_b_results$single_predictor

  print(res_phvas_exam_b_fi3$radar_plot)
  print(res_phvas_exam_b_fi3$composite_table_export)
  print(res_phvas_exam_b_mmt8$radar_plot)
  print(res_phvas_exam_b_mmt8$composite_table_export)
  print(res_phvas_exam_b_single$radar_plot)
  print(res_phvas_exam_b_single$composite_table_export)

  message("vis01_exam_b exported to: ", normalizePath(file.path("output", "exam_incl", "fi3", "ph_vas"), winslash = "/", mustWork = FALSE))
}
