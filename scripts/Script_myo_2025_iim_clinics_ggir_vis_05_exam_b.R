# ************************************************************
# TIS radar comparisons for GGIR / fi3 / mmt8 models
...
prepare_ggir_tis_data_vis05 <- function(ggir_data, tis_data) {
  ...
  fi3_col <- resolve_first_existing_vis05(merged_data, c("fi3", "fi_3"), "fi3")
...
run_tis_workflow_vis05_exam_b <- function(
  ...
  dir.create(file.path("output", "exam_incl", "fi3", "tis", "tables"), recursive = TRUE, showWarnings = FALSE)
  dir.create(file.path("output", "exam_incl", "fi3", "tis", "figures"), recursive = TRUE, showWarnings = FALSE)
  ...
  if (isTRUE(export_results)) {
    prefix <- paste0(format(Sys.Date(), "%y%m%d"), "_", sanitize_name_vis05(response), "_tis_fi3")

    export_paths <- list(
      fi3 = export_result_vis05(
        result = res_fi3,
        table_file = file.path("output", "exam_incl", "fi3", "tis", "tables", paste0(prefix, "_fi3adj_exam_b_", suffix, ".xlsx")),
        plot_file = file.path("output", "exam_incl", "fi3", "tis", "figures", paste0(prefix, "_fi3adj_exam_b_", suffix, ".png"))
      ),
      mmt8 = export_result_vis05(
        result = res_mmt8,
        table_file = file.path("output", "exam_incl", "fi3", "tis", "tables", paste0(prefix, "_mmt8adj_exam_b_", suffix, ".xlsx")),
        plot_file = file.path("output", "exam_incl", "fi3", "tis", "figures", paste0(prefix, "_mmt8adj_exam_b_", suffix, ".png"))
      ),
      single = export_result_vis05(
        result = res_single,
        table_file = file.path("output", "exam_incl", "fi3", "tis", "tables", paste0(prefix, "_single_exam_b_", suffix, ".xlsx")),
        plot_file = file.path("output", "exam_incl", "fi3", "tis", "figures", paste0(prefix, "_single_exam_b_", suffix, ".png"))
      )
    )
  }
...
if (exists("d18_ggir_data") && exists("d18_tis_comp_01")) {
  ggir_vis05_exam_b_results <- run_tis_workflow_vis05_exam_b(
    ggir_data = d18_ggir_data,
    tis_data = d18_tis_comp_01,
    export_results = isTRUE(getOption("myo.ggir.vis05.exam_b.export", TRUE))
  )

  res_tis_exam_b_fi3 <- ggir_vis05_exam_b_results$fi3_adjusted
  res_tis_exam_b_mmt8 <- ggir_vis05_exam_b_results$mmt8_adjusted
  res_tis_exam_b_single <- ggir_vis05_exam_b_results$single_predictor

  print(res_tis_exam_b_fi3$radar_plot)
  print(res_tis_exam_b_fi3$composite_table_export)
  print(res_tis_exam_b_mmt8$radar_plot)
  print(res_tis_exam_b_mmt8$composite_table_export)
  print(res_tis_exam_b_single$radar_plot)
  print(res_tis_exam_b_single$composite_table_export)

  message("vis05_exam_b exported to: ", normalizePath(file.path("output", "exam_incl", "fi3", "tis"), winslash = "/", mustWork = FALSE))
}
