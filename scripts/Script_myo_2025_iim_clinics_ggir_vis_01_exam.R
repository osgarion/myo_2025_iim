# ************************************************************
# PhVAS radar comparisons for GGIR / fi2 / mmt8 models
...
prepare_ggir_phvas_data_vis01_exam <- function(ggir_data) {
  ...
  fi2_col <- resolve_first_existing_vis05(ggir_data, c("fi2", "fi_2"), "fi2")
...
run_phvas_workflow_vis01_exam <- function(
  ...
  dir.create(file.path("output", "exam_incl", "ph_vas", "tables"), recursive = TRUE, showWarnings = FALSE)
  dir.create(file.path("output", "exam_incl", "ph_vas", "figures"), recursive = TRUE, showWarnings = FALSE)
  ...
  if (isTRUE(export_results)) {
    prefix <- paste0(format(Sys.Date(), "%y%m%d"), "_", sanitize_name_vis05(response), "_phvas")

    export_paths <- list(
      fi2 = export_result_vis05(
        result = res_fi2,
        table_file = file.path("output", "exam_incl", "ph_vas", "tables", paste0(prefix, "_fi2adj_exam_", suffix, ".xlsx")),
        plot_file = file.path("output", "exam_incl", "ph_vas", "figures", paste0(prefix, "_fi2adj_exam_", suffix, ".png"))
      ),
      mmt8 = export_result_vis05(
        result = res_mmt8,
        table_file = file.path("output", "exam_incl", "ph_vas", "tables", paste0(prefix, "_mmt8adj_exam_", suffix, ".xlsx")),
        plot_file = file.path("output", "exam_incl", "ph_vas", "figures", paste0(prefix, "_mmt8adj_exam_", suffix, ".png"))
      ),
      single = export_result_vis05(
        result = res_single,
        table_file = file.path("output", "exam_incl", "ph_vas", "tables", paste0(prefix, "_single_exam_", suffix, ".xlsx")),
        plot_file = file.path("output", "exam_incl", "ph_vas", "figures", paste0(prefix, "_single_exam_", suffix, ".png"))
      )
    )
  }
...
if (exists("d18_ggir_data")) {
  ggir_vis01_exam_results <- run_phvas_workflow_vis01_exam(
    ggir_data = d18_ggir_data,
    export_results = isTRUE(getOption("myo.ggir.vis01.exam.export", TRUE))
  )

  res_phvas_exam_fi2 <- ggir_vis01_exam_results$fi2_adjusted
  res_phvas_exam_mmt8 <- ggir_vis01_exam_results$mmt8_adjusted
  res_phvas_exam_single <- ggir_vis01_exam_results$single_predictor

  print(res_phvas_exam_fi2$radar_plot)
  print(res_phvas_exam_fi2$composite_table_export)
  print(res_phvas_exam_mmt8$radar_plot)
  print(res_phvas_exam_mmt8$composite_table_export)
  print(res_phvas_exam_single$radar_plot)
  print(res_phvas_exam_single$composite_table_export)

  message("vis01_exam exported to: ", normalizePath(file.path("output", "exam_incl", "ph_vas"), winslash = "/", mustWork = FALSE))
}
