...
## optional rename vector from new MDA label sheet ----
pls_rename_vec <- tryCatch({
  mda_labels <- import(path_clin_01, which = "260507_MDAcategories ") |>
    janitor::clean_names() |>
    mutate(abbreviation = janitor::make_clean_names(abbreviation))
  rv <- mda_labels$abbreviation
  names(rv) <- mda_labels$name_en
  rv
}, error = function(e) {
  message("Sheet '260507_MDA categories' not found — using raw column names.")
  character(0)
})
