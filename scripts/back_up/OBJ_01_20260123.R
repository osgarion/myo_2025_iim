...
### ggir ----
# *******************************************************
d10_legend <- rio::import("R:/MYOZITIDY_VÝZKUM/_IMMET-GRANT/IMMET štatistika/Data/Raw/Geneactiv/merged_table_220126.xlsx",
                          which="legend")
d10_ostatni <- rio::import("R:/MYOZITIDY_VÝZKUM/_IMMET-GRANT/IMMET štatistika/Data/Raw/Geneactiv/merged_table_220126.xlsx",
                           which="ostatni")
d10_cas <- rio::import("R:/MYOZITIDY_VÝZKUM/_IMMET-GRANT/IMMET štatistika/Data/Raw/Geneactiv/merged_table_220126.xlsx",
                           which="cas") |> 
...
d10_aktivita <- rio::import("R:/MYOZITIDY_VÝZKUM/_IMMET-GRANT/IMMET štatistika/Data/Raw/Geneactiv/merged_table_220126.xlsx",
                           which="aktivita")|> 
  rename(any_of(setNames(d10_legend$names_orig, d10_legend$names_new))) |> 
  separate(
    file_name,
...
d11_clinics <- rio::import("R:/MYOZITIDY_VÝZKUM/_IMMET-GRANT/IMMET štatistika/Data/Raw/Geneactiv/Klinicka_tabulka_13012026.xlsx",
  ...
  relocate(immet_id, exam_order, disease_subtype, contains("response"),
           smoker_100_cigs) |> 
  group_by(immet_id) |>
  tidyr::fill(response_m0_m6, response_m0_m18, smoker_100_cigs, .direction = "downup") |>
  ungroup() |> 
  mutate(
    across(
      where(\(x) {
    ...
    across(.cols = immet_id:smoker_100_cigs, as.factor)
  )
...
d11_clinics_imp <- d11_clinics |> 
  ...
  select(
    where(~ mean(is.na(.x)) <= 0.15)
  ) |> 
  mice(m = 20, maxit = 20, method = "pmm", seed = 123, printFlag = FALSE) |> 
  mice::complete() |> 
