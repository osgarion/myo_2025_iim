# ************************************************************
# SF-36 radar comparisons for GGIR / fi3 / mmt8 models
...
prepare_ggir_sf36_data_vis03 <- function(data) {
  ...
  fi3_col <- resolve_first_existing_vis03(data, c("fi3", "fi_3"), "fi3")
...
run_sf36_workflow_vis03_exam_b <- function(
    data = d18_ggir_data,
    sf36_outcomes = c(
      "sf36_pcs", "sf36_mcs", "sf36pf", "sf36rp", "sf36bp",
      "sf36gh", "sf36vt", "sf36sf", "sf36re", "sf36mf"
  ...
  dir.create(file.path("output", "exam_incl", "fi3", "sf36", "tables"), recursive = TRUE, showWarnings = FALSE)
  dir.create(file.path("output", "exam_incl", "fi3", "sf36", "figures"), recursive = TRUE, showWarnings = FALSE)

  results <- map(sf36_available, function(outcome_name) {
    res_fi3 <- compare_adjusted_models_vis03(
      data = prepared_data,
    ...
    if (isTRUE(export_results)) {
      prefix <- paste0(format(Sys.Date(), "%y%m%d"), "_", sanitize_name_vis03(outcome_name), "_sf36_fi3")

      export_paths <- list(
        fi3 = export_result_vis03(
          result = res_fi3,
          table_file = file.path("output", "exam_incl", "fi3", "sf36", "tables", paste0(prefix, "_fi3adj_exam_b_", suffix, ".xlsx")),
          plot_file = file.path("output", "exam_incl", "fi3", "sf36", "figures", paste0(prefix, "_fi3adj_exam_b_", suffix, ".png"))
        ),
        mmt8 = export_result_vis03(
          result = res_mmt8,
          table_file = file.path("output", "exam_incl", "fi3", "sf36", "tables", paste0(prefix, "_mmt8adj_exam_b_", suffix, ".xlsx")),
          plot_file = file.path("output", "exam_incl", "fi3", "sf36", "figures", paste0(prefix, "_mmt8adj_exam_b_", suffix, ".png"))
        ),
        single = export_result_vis03(
          result = res_single,
          table_file = file.path("output", "exam_incl", "fi3", "sf36", "tables", paste0(prefix, "_single_exam_b_", suffix, ".xlsx")),
          plot_file = file.path("output", "exam_incl", "fi3", "sf36", "figures", paste0(prefix, "_single_exam_b_", suffix, ".png"))
        )
      )
    }
...
if (exists("d18_ggir_data")) {
  ggir_vis03_exam_b_results <- run_sf36_workflow_vis03_exam_b(
    data = d18_ggir_data,
    export_results = isTRUE(getOption("myo.ggir.vis03.exam_b.export", TRUE))
  )

  message("vis03_exam_b exported to: ", normalizePath(file.path("output", "exam_incl", "fi3", "sf36"), winslash = "/", mustWork = FALSE))
}
