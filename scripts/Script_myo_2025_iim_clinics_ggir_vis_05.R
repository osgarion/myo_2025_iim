...
# Combines the ideas from vis_01 and vis_02, but replaces PhVAS
# and sf36 outcomes with tis_total merged from d18_tis_comp_01.
# ************************************************************
...
compare_adjusted_models_vis05 <- function(data, response, base_predictor, tested_parameters, ci = 0.95) {
  models <- map(tested_parameters, function(parameter_name) {
    model_data <- data |>
      select(immet_id, age_m0, sex, all_of(response), all_of(base_predictor), all_of(parameter_name)) |>
      na.omit()
  ...
  list(
    ...
    radar_plot = build_fmsb_radar_vis05(
      ...
      subtitle = paste0(
        ...
        " + tested variable + age_m0 + sex + (1 | immet_id)"
      )
    )
  )
}
...
compare_single_predictor_models_vis05 <- function(data, response, tested_parameters, ci = 0.95) {
  models <- map(tested_parameters, function(parameter_name) {
    model_data <- data |>
      select(immet_id, age_m0, sex, all_of(response), all_of(parameter_name)) |>
      na.omit()

    fml <- stats::as.formula(
      paste0(response, " ~ ", parameter_name, " + age_m0 + sex + (1 | immet_id)")
    )
  ...
  list(
    ...
    radar_plot = build_fmsb_radar_vis05(
      ...
      title = paste0("Radar comparison of focal predictors: ", response),
      subtitle = paste0(
        response, " ~ tested variable + age_m0 + sex + (1 | immet_id)"
      )
    )
  )
}
...
run_tis_workflow_vis05 <- function(
  ...
  dir.create(file.path("output", "tis", "tables"), recursive = TRUE, showWarnings = FALSE)
  dir.create(file.path("output", "tis", "figures"), recursive = TRUE, showWarnings = FALSE)
  ...
  if (isTRUE(export_results)) {
    ...
    export_paths <- list(
      fi2 = export_result_vis05(
        result = res_fi2,
        table_file = file.path("output", "tis", "tables", paste0(prefix, "_fi2adj_", suffix, ".xlsx")),
        plot_file = file.path("output", "tis", "figures", paste0(prefix, "_fi2adj_", suffix, ".png"))
      ),
      mmt8 = export_result_vis05(
        result = res_mmt8,
        table_file = file.path("output", "tis", "tables", paste0(prefix, "_mmt8adj_", suffix, ".xlsx")),
        plot_file = file.path("output", "tis", "figures", paste0(prefix, "_mmt8adj_", suffix, ".png"))
      ),
      single = export_result_vis05(
        result = res_single,
        table_file = file.path("output", "tis", "tables", paste0(prefix, "_single_", suffix, ".xlsx")),
        plot_file = file.path("output", "tis", "figures", paste0(prefix, "_single_", suffix, ".png"))
      )
    )
  }
...
if (exists("d18_ggir_data") && exists("d18_tis_comp_01")) {
  ggir_vis05_results <- run_tis_workflow_vis05(
    ggir_data = d18_ggir_data,
    tis_data = d18_tis_comp_01,
    export_results = isTRUE(getOption("myo.ggir.vis05.export", TRUE))
  )

  res_tis_fi2 <- ggir_vis05_results$fi2_adjusted
  res_tis_mmt8 <- ggir_vis05_results$mmt8_adjusted
  res_tis_single <- ggir_vis05_results$single_predictor

  print(res_tis_fi2$radar_plot)
  print(res_tis_fi2$composite_table_export)
  print(res_tis_mmt8$radar_plot)
  print(res_tis_mmt8$composite_table_export)
  print(res_tis_single$radar_plot)
  print(res_tis_single$composite_table_export)

  message("vis05 exported to: ", normalizePath(file.path("output", "tis"), winslash = "/", mustWork = FALSE))
}
