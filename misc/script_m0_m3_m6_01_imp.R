...
# path_export <- "R:/MYOZITIDY_VÝZKUM/_IMMET-GRANT/IMMET štatistika/Output/251002/m0_m3_m6_imp/"
path_export <- "R:/MYOZITIDY_VÝZKUM/_IMMET-GRANT/IMMET štatistika/Output/251017/m0_m3_m6_imp/"
...
multidplyr::cluster_copy(cluster, c("model_covar_3","model_min_3", "model_comp_int"))
...
res_mixMod_covar_02 <- d04_sel2 |>
  ...
  filter(poradie_vysetrenia %in% c("M0","M3", "M6")) |> 
  mice::mice("pmm", m = 20,
       maxit = 15,
  ...
  do(
    data = data.table(.),
    mod_without = model_min_3(., filter_na = FALSE),
    mod_covar_gk = model_covar_3(., "denna_davka_gk_mg_kg_bw", filter_na = FALSE),
    mod_covar_age = model_covar_3(., "vek", filter_na = FALSE),
    mod_covar_bcm = model_covar_3(., "bcm", filter_na = FALSE),
    mod_covar_creatine = model_covar_3(., "kreatinin_umol_l", filter_na = FALSE)
  ) |> 
  collect()
